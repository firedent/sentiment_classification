...
class rnn(torch.nn.Module):
    ...
    def rnnCell(self, input, hidden):
        ...
        """
        Tanh = torch.nn.Tanh()
        return Tanh(self.ih(input) + self.hh(hidden))
    ...
    def forward(self, input):
        ...
        """
        hidden = torch.zeros(input.size(1), 128)
        for batch in input:
            hidden = self.rnnCell(batch, hidden)
        return hidden
...
def lstm(input, hiddenSize):
    ...
    """
    lstm = torch.nn.LSTM(input.size(2), hiddenSize, batch_first=True)
    return lstm(input)
...
def conv(input, weight):
    ...
    """
    return torch.conv1d(input, weight)
